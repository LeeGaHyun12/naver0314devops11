...
public class Ex8_String {
	...
	public static void main(String[] args) {
		...
		System.out.println(str1.charAt(0)); //7번 인덱스의 문자: H
		...
		System.out.println(str2==str3); //객체시 주소를 비교: 주소가 다르므로 false(==동등연산자는 기본값 비교시에만 사용ㅇ)

		//문자열 추출 함수 "Have a nice day"
		System.out.println(str1.substring(7)); //7번 인덱스부터 끝까지 추출: Nice day 
		System.out.println(str1.substring(7,10)); //7부터 9까지 출력
		...
		System.out.println(str1.toLowerCase().endsWith("day")); //str1을 소문자로 변환후 day로 끝나는지 비교: true

		//compareTo 로 비교시 
		System.out.println("apple".compareTo("append"));// 'l','e': 7(양수:첫문자열이 더 크다), 첫문자열이 작을경우 음수가 나온다
	    System.out.println("apple".compareTo("banana")); // 'a','b' 비교: -1 ->순서대로 오름차순이라는뜻
	    System.out.println("apple".compareTo("apple")); //완전히 같을 경우 0
	    
		
		//반복 
		System.out.println(str2.repeat(3)); // 3번 반복하겠다
		...
		System.out.println(a1[0]+","+a1[4]); //0번과 4번 배열값 출력 
		
		//분리하는 또다른 방법 
		StringTokenizer st=new StringTokenizer(colors,",");
		System.out.println("분리할 토큰수: "+st.countTokens());
		//분리된 토큰값 출력
		while(st.hasMoreTokens())// 더이상 토큰이 없을 경우 false가 되면서 while문을 빠져나간
		{ 
