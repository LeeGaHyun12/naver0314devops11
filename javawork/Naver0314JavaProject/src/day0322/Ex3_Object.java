...
class MyShop{
	...
	static final String SHOP="24시간이마트";  
	...
	//overloading method(중복함수): 메서드명을 같으나 인자가 달라야 한다 
	public void setData(String sangpum, int su,int dan,String message) {
		...
		//인스턴스메서드에서 static 메서드 호출이 가능함
		//반대로 static메서드에서 인스턴스 멤버메서드를 호출할 수 없음 
		//static 메서드에서 같은 static 메서드는 호출이 가능함. 
		setMessage(message); //앞에 클래스명은 같은 클래스라 생략함 
		
	}
	
	//출력은 getter가 아닌 일반멤버메서드로 만들어보자
...
public class Ex3_Object {
	...
	public static void main(String[] args) {
		...
		sang1.setSu(3);
		sang1.setdan(2300);
		MyShop.setMessage("오늘 각종 과일 세일합니다"); //static 메서드는 클래스명으 호출
		...
		sang3.setData("참외",5, 2000,"오늘세일마감!!!");
		sang3.writeData();
