...
public class Ex5_ClientChat extends JFrame implements ActionListener,Runnable{
	...
	public void run() {
		...
		while(true) {
			...
			try {
				...
				//채팅창에 출력
				area.append(msg+"\n");
				this.autoScroll(); //항상 마지막 문자열 보이게 하기
			}catch(IOException e)
			{
				e.printStackTrace();}
			}
	...
	public void actionPerformed(ActionEvent e) {
		...
		//메세지입력창이거나 전송버튼일 경우 서버로 메세지 보내기
	...
	public void initDesign()
	{
		...
		area=new JTextArea();
		//area.setBounds(10,40,400,400);
		JScrollPane pane=new JScrollPane(area);
		pane.setBounds(15,40,470,350);
		this.add(pane);
		...
		btnConnect.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				// TODO Auto-generated method stub
				String serverIp=JOptionPane.showInputDialog("접속할 서버 IP 입력하세요","192.168.0.59");//초기값은 로컬 IP
				nickName=JOptionPane.showInputDialog("접속할 닉네임을 입력하세요","가혀니");
				...
				try {
					...
					//서버로 "1|닉네임"의 형태로 보내기
					pw.write("1| "+nickName+"\n");
					pw.flush();//flush를 해야 비로소 서버로 전송이 된다. +접속 성공했을 때 한 번만 보내는 부분
